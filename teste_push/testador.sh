...
echo "============================================="
echo "   VALIDAÇÃO COM BINÁRIO EXTERNO (42 CHECKER) "
echo "============================================="

# 1. Compilar apenas o SEU push_swap
...
# 2. Verificar se o binário enviado existe e é executável
if [ -x "$CHECKER_BIN" ]; then
    echo -e "Verificação do Checker Enviado: ${GREEN}[OK]${RESET}"
else
    echo -e "Erro: O arquivo '$CHECKER_BIN' não foi encontrado ou não possui permissão (+x)."
    exit 1
fi

echo "============================================="

# Teste 1: Lista de 3 Números (Limite: <= 3 ops)
ARG_3=$(seq -100 100 | shuf -n 3 | tr '\n' ' ')
OPS_3=$(./push_swap $ARG_3 | wc -l)
CHECK_3=$(./push_swap $ARG_3 | $CHECKER_BIN $ARG_3)
...
if [ "$CHECK_3" = "OK" ] && [ $OPS_3 -le 3 ]; then
    ...
    echo -e "-> Resultado: ${RED}[FALHOU]${RESET} (Checker deu: $CHECK_3)"
fi

echo "============================================="

# Teste 2: Lista de 5 Números (Limite: <= 12 ops)
ARG_5=$(seq -100 100 | shuf -n 5 | tr '\n' ' ')
OPS_5=$(./push_swap $ARG_5 | wc -l)
CHECK_5=$(./push_swap $ARG_5 | $CHECKER_BIN $ARG_5)
...
if [ "$CHECK_5" = "OK" ] && [ $OPS_5 -le 12 ]; then
    ...
    echo -e "-> Resultado: ${RED}[FALHOU]${RESET} (Checker deu: $CHECK_5)"
fi
...
CHECK_100=$(./push_swap $ARG_100 | $CHECKER_BIN $ARG_100)
...
if [ "$CHECK_100" = "OK" ]; then
    ...
    echo -e "-> Resultado: ${RED}[CRÍTICO - KO] O algoritmo errou a ordenação!${RESET}"
fi
...
CHECK_500=$(./push_swap $ARG_500 | $CHECKER_BIN $ARG_500)

echo "Teste com 500 números aleatórios..."
echo "-> Operações: $OPS_500 (Meta Mínima: <= 5500)"
if [ "$CHECK_500" = "OK" ] && [ $OPS_500 -le 5500 ]; then
    echo -e "-> Resultado: ${GREEN}[PASSOU - OK] Sistema estável!${RESET}"
else
    echo -e "-> Resultado: ${RED}[FALHOU] Acima de 5500 ops ou KO (Checker deu: $CHECK_500)${RESET}"
fi
echo "============================================="
